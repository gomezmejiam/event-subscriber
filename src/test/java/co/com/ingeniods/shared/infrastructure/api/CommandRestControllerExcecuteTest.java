...
@ActiveProfiles("test")
class CommandRestControllerExcecuteTest {
	...
	@Test
	void excecuteWithError() throws Exception {
		
		String eventRequest = getEventResource("1");
		MockHttpServletRequestBuilder request = MockMvcRequestBuilders.post("/command/excecute/").content(eventRequest)
		...
		assertEquals(true, eventResponse.getError().contains("ENTITY_ALREADY_REGISTRED"));
		
	}

	@Test
	void excecuteWithoutError() throws Exception {
		List<EmployeeDTO> resultPre = new ArrayList<EmployeeDTO>();
		employeeRepository.findAll().forEach(resultPre::add);
		assertEquals(1, resultPre.size());
		String eventRequest = getEventResource("2");
		MockHttpServletRequestBuilder request = MockMvcRequestBuilders.post("/command/excecute/").content(eventRequest)
				.contentType(MediaType.APPLICATION_JSON);
		String response = restApi.perform(request).andExpect(status().isOk()).andReturn().getResponse()
				.getContentAsString();
		...
		assertEquals(2, resultPost.size());
	}
	
	@Test
	void excecuteWithoutProcessor() throws Exception {
